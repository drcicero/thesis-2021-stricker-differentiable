\chapter{Forward Mode Differentiation}
...
\section{Code replacement with macros}
...
def d(t: Expr[Term])(using Quotes): Expr[Term] = t match
    ...
    case '{ $v: V } => '{ 0 }               // constant
\end{lstlisting}
As this was just a small experiment we leave this here without much explanation for now. \todo{maybe comments enough explanation? Or just explain more} In fact we wouldn't even need macros and could just write our term with algebraic data types and recursively match them at runtime to implement this approach. This would reduce the boilerplate in comparison to the macro-implementation above.


\section{Operator overloading with dual numbers}
...
\lstscalacaption{implementation/DualNumber.scala}{3}{12}{Dual number implementation}{lst:forwardDualNumber}
...
\section{Static typing taken too far (Or: Let the compiler code for you)}
...
If we combine all functionalities described above we gain the possibility to define a function and derive its derivative \todowording entirely at type level:
\begin{lstlisting}
